<?php
	...
	class Painel
	{
		...
		public static function loadJS($files,$page){
			...
			if($page == $url){
				foreach ($files as $key => $value) {
					echo '<script src="'.INCLUDE_PATH_PAINEL_COLABORADO.'js/'.$value.'"></script>';
				}
			}
		}
		...
		public static function logadoconsumido(){
		...
		public static function colaborado_logado(){
			return isset($_SESSION['login_colaborado']) ? true : false;
		}

		public static function colaborado_loggout(){
			setcookie('lembrar','true',time()-1,'/');
			session_destroy();
		...
		public static function carregarPaginaLoja(){
			if(isset($_GET['url'])){
		...
		public static function carregarPaginaColaborado(){
			if(isset($_GET['url'])){
				$url = explode('/',$_GET['url']);
				if(file_exists('pages/'.$url[0].'.php')){
					include('pages/'.$url[0].'.php');
		...
		public static function alert($tipo,$mensagem){
			if($tipo == 'sucesso'){
				echo '<div class="box-alert sucesso center"><i class="fa fa-check"></i> '.$mensagem.'</div>';
			}else if($tipo == 'erro'){
				echo '<div class="box-alert erro center"><i class="fa fa-times"></i> '.$mensagem.'</div>';
			}else if($tipo == 'atencao'){
				echo '<div class="box-alert atencao center"><i class="fas fa-exclamation-triangle"></i> '.$mensagem.'</div>';
			}
		}
		...
		public static function uploadFile($file){
			...
			if(move_uploaded_file($file['tmp_name'],BASE_DIR_PAINEL_LOJA.'/uploads/'.$imagemNome))
				return $imagemNome;
			else
				return false;
		}
		...
		public static function uploadPerfil($file){
			$formatoArquivo = explode('.',$file['name']);
			$imagemNome = uniqid().'.'.$formatoArquivo[count($formatoArquivo) - 1];
			if(move_uploaded_file($file['tmp_name'],BASE_DIR_PAINEL_COLABORADO.'/uploads/'.$imagemNome))
				return $imagemNome;
			else
				return false;
		}
		...
		public static function uploadLogo($file){
			$formatoArquivo = explode('.',$file['name']);
			$imagemNome = uniqid().'.'.$formatoArquivo[count($formatoArquivo) - 1];
			if(move_uploaded_file($file['tmp_name'],BASE_DIR_PAINEL_LOJA.'/logo/'.$imagemNome))
				return $imagemNome;
			else
				return false;
		}
